...
## Key globals (declared in ui.js — changelog)

| Symbol | Purpose |
|---|---|
| `CHANGELOG` | Hardcoded version history array — **update this when adding features** |
| `CHANGELOG_KEY` | localStorage key `'forza_changelog_read'` — stores last-seen version |
| `changelogBtn/Dot/Drop` | Bell icon DOM refs |

`CHANGELOG[0].version` is the "latest" version. Dot shows when `localStorage` value differs. Update `CHANGELOG` in `js/ui.js` at line ~68 when shipping new features.
...
- **Changelog**: Bell icon top-right. Red dot on unread updates (localStorage). Dropdown with version history. Hardcoded in `CHANGELOG` array (`js/ui.js`).
...
Before committing, update this file to reflect any changes:
- New features → add to **Features** list
- New globals → add to relevant globals table
- New files → add to **Structure** block
- New changelog entry → prepend to `CHANGELOG` array in `js/ui.js` and bump `version`
